...
//5)docker + k8s + redis
//6)gitlab ci/cd
//7)rabbitmq/kafka - подумать только дл€ чего
//8)Ќу и получение данных :)
...
builder.Services.AddQuartz(q =>
{
    ...
    q.AddTrigger(opts => opts
        ...
        .WithIdentity("trigger1", "group1")     // идентифицируем триггер с именем и группой
                .StartAt(new DateTimeOffset(DateTime.UtcNow).AddSeconds(5))                            // запуск сразу после начала выполнени€
                .WithSimpleSchedule(x => x            // настраиваем выполнение действи€
